#!/usr/bin/env Rscript
# 03_run_pavo_analysis.R
#
# Run pavo adjacency analysis on exemplar color images.
# Key modification from 2019: Uses species-specific gestalt k (not fixed k=4)
#
...
# Output
adjacency_csv <- file.path(output_dir, "pavo_adjacency_color.csv")
classified_rds <- file.path(output_dir, "classified_images.rds")
...
cat("=" , rep("=", 59), "\n", sep = "")
cat("Pavo Adjacency Analysis - Color Images\n")
cat("=" , rep("=", 59), "\n\n", sep = "")
...
for (i in 1:nrow(exemplars)) {
  ...
  img_path <- exemplars$image_path[i]
